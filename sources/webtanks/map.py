...
class map():
	...
	def search_break_walls_for_bullet(self, kind, X, Y):
		...
		if(kind == 3):
			for i in range(0, len(self.breakhorwall), 3):
				if(math.fabs(Yd - self.breakhorwall[i]) < 10 and self.breakhorwall[i + 1] + 6 <= X and self.breakhorwall[i + 1] + self.breakhorwall[i + 2] >= Xd + 6):
					return (3, i)
			return (0, 0)
		if(kind == 4):
			for i in range(0, len(self.breakhorwall), 3):
	...
	def breakwall(self, X, Y, kind):
		...
		if (type == 1 or type == 2):
			...
			self.breakvertwall[number + 2] = Yd - self.breakvertwall[number + 1] + 5
		if (type == 3 or type == 4):
			self.breakhorwall.append(self.breakhorwall[number])
			self.breakhorwall.append(Xd - 11)
			self.breakhorwall.append(self.breakhorwall[number + 1] + self.breakhorwall[number + 2] - Xd)
			self.breakhorwall[number + 2] = Xd - self.breakhorwall[number + 1] + 5
		res = [0, 0]
